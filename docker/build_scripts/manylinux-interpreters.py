...
def add_parser_list(subparsers):
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="display additional information (--format=text only, ignored for --format=json)",
    )
    parser.add_argument(
        "-i",
        "--installed",
        default=False,
        action="store_true",
        help="only list installed interpreters",
    )
    parser.add_argument(
        "--format",
        choices=["text", "json"],
        default="text",
        help="text is not meant to be machine readable (i.e. the format is not stable)",
    )
...
def _list(args):
    ...
    for tag in tags:
        ...
        info = get_info_from_path(install_path) if installed else get_info_from_tag(tag)
        tag_info = {
            "identifier": tag,
            "installed": installed,
    ...
    for tag in tag_infos:
        ...
        if args.verbose:
            ...
            print(f"  implementation:         {tag['implementation']}")
            print(f"  implementation version: {tag['implementation_version']}")
...
def add_parser_ensure(subparsers):
    ...
    parser.add_argument(
        "tags",
        choices=get_all_tags(no_check=NO_CHECK),
        metavar="TAG",
        nargs="+",
        help="tag with format '<python tag>-<abi tag>' e.g. 'pp310-pypy310_pp73'",
    )
...
def ensure_all(args):  # noqa: ARG001
